...
level_rows AS (
    SELECT 'Aggs' AS UseCase, 'ACCOUNT' AS AggregationLevel,
        CONCAT('ACCOUNT_', CAST(OEDID AS STRING), '_', CAST(AccNumber AS STRING)) AS AggregationValue,
        CAST(NULL AS STRING) AS AggregationCountry, CAST(NULL AS STRING) AS AggregationState,
        CAST(NULL AS STRING) AS AggregationRegion, CAST(NULL AS STRING) AS AggregationPostcode,
        rows.* FROM complete_rows AS rows

    UNION ALL
    SELECT 'Aggs', 'COUNTRY', COALESCE(CountryCode, 'UNKNOWN'),
        CountryName, NULL, NULL, NULL, rows.* FROM complete_rows AS rows

    UNION ALL
    SELECT 'Aggs', 'STATE', CONCAT('US_', REGEXP_REPLACE(UPPER(StateValue), '[^A-Z0-9]+', '_')),
        CountryName, StateValue, NULL, NULL, rows.*
    ...
    SELECT 'Aggs', 'POSTCODE',
        ...
        CountryName, StateValue, NULL, PostcodeValue, rows.*
...
ORDER BY
    OEDID, UseCase, AggregationSortOrder, AggregationValue,
    ReportingEntity, PerilCode, SourceCurrency;

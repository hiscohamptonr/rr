...
- `Dataiku-Aggs.sql` is unchanged from commit `d264675` on the merged
  `fet-dataiku-aggs` branch.
- It currently produces gross TIV aggregates from OEDIDs 42 and 44.
- It does not currently calculate participation-adjusted gross exposure or
  QS/SRP-adjusted net exposure.
- It does not have confirmed policy, peril, deductible, policy-limit, PRA
  region, CDS class, or CRESTA inputs.

## Objective

Make the existing query a controlled, reusable **uncapped OED exposure**
aggregation using only columns and mapping sources already referenced by the
script.

The result should support provisional Aggs, PRA, and BSCR geography views. It
must not claim to calculate capped exposure or modeled PML.
...
Compare at common, uncapped grains first:

1. source population and reporting period;
2. row, account, and location counts;
3. ground-up exposure before policy terms;
4. participation-adjusted gross exposure;
5. QS/SRP-adjusted net exposure;
6. totals by country, entity, geocoding, and available regional views.

Do not expect the Dataiku results to equal the final `BSCR_UKEU.py` capped
output because the current Dataiku source does not contain the required policy
terms. Record deductible, limit, policy-join, source-population, FX, and mapping

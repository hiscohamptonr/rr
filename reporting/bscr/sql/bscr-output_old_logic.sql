-- BSCR diagnostic comparison: legacy policy-side geocoding joins.
-- NOT a production correction or a complete reproduction of the legacy query.
-- Reintroduces one policy row per account geocode flag, then assigns all
-- matching account/peril exposure to each policy-side geocode bucket.
-- The policy-to-location join below intentionally matches account only.
-- The exposure rejoin retains the current peril match to isolate geocoding;
-- unlike the original query, it does not apply other-peril policy terms.
-- Current policy-ID cap grouping, routing, FX and retention are unchanged.
-- Run beside bscr-output.sql on the SAME snapshot and parameter values.
...
policies AS (
    ...
    INNER JOIN loc_exposure
        ON loc_exposure.accgrpid = policy.accgrpid

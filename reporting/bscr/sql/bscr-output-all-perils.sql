-- BSCR diagnostic comparison: both perils for every geographic region.
-- Produces one row per country/entity/region/geocode/peril; does NOT sum perils.
-- Includes both perils in ALL and all regional labels, unlike production routing.
-- For max-peril reconciliation, first sum countries by entity/region/geocode
-- separately for each peril, then compare the two gross totals and net totals.
-- A maximum of regional totals is not a sum of location/policy-level maxima.
-- Run on the same source snapshot and parameters as other comparison queries.
...
-- Every geographic region is evaluated for earthquake (1) and wind (2).
...
region_peril_lookup AS (
    SELECT region, selected_perils.peril_id, source_flag
    FROM (VALUES
        ('is_nahu',   'is_nahu'),
        ('is_eu',     'is_eu'),
        ('is_jp',     'is_jp'),
        ...
        ('is_us_all', 'is_us_all'),
        ('is_non_us', 'is_non_us')
    ) AS mapped_regions(region, source_flag)
    CROSS JOIN peril_selection AS selected_perils
),
loc_tiv AS (
    SELECT
        locid,

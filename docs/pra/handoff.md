...
|---|---|---|---|
| Canonical SQL route | `pra/sql/pra-earthquake.sql`, approved EDM snapshot and peril/policy parameters | PRA raw and aggregate earthquake inputs; validate output headers and totals before load | Raw/aggregate workbook tabs |
| Legacy route | `pra/sql/aggs-from-edm.sql`, embedded `sql` sheet | Historical hard-coded 2/2 route; do not use for a current cycle | None |
